...
Term operator+(const Term &f, const Term &s)
{
  ...
  {
    std::cerr << " can't add terms, first term returned ";
    return f;
  }
}
...
std::istream &operator>>(std::istream &in, Polynomial &pol)
{
  ...
  in.getline(input, POLYNOM_INPUT);
  ...
  {
    if (finalws[i] == '+' || finalws[i] == '-' || finalws[i] == '\0') 
    {
      ...
      {
        if (xIndex == 0 || (xIndex == 1 && (termString[0] == '-' || termString[0] == '+')))
        {
        ...
        for (int j = xIndex + 1; termString[j] != '\0'; ++j)
        {
          if (termString[j] == '^')
          {
            expIndex = j;
